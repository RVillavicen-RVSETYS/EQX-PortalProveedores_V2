...
class ExentoAnoFisc_Mdl
{
    private $db;
    private static $debug = 0;
    private $tabla = 'conf_provExentoAnoFisc';
    private $logFile = 'exentoAnioFiscal.log';

    public function __construct()
    {
        if (self::$debug) {
            echo "<h2>Ya estamos dentro de la Clase ExentoAnoFisc_Mdl.</h2>";
        }
    ...
    public function obtenerExentos()
    {
        ...
        try {
            ...
            error_log("[$timestamp] app/Models/Proveedores/Excepciones/ExentoAnoFisc_Mdl.php -> Error Al Obtener Proveedores Exentos Año Fiscal: " . $e->getMessage(), 3, LOG_FILE_BD);
    ...
    public function getProveedores()
    {
        try {
            ...
            $timestamp = date("Y-m-d H:i:s");
            error_log("[$timestamp] app/Models/Proveedores/Excepciones/ExentoAnoFisc_Mdl.php -> Error Al Obtener Proveedores: " . $e->getMessage(), 3, LOG_FILE_BD);
            return ['success' => false, 'message' => 'Error Al Obtener Proveedores. Notifica a tu administrador'];
        }
    }

    public function registraExentoAnoFisc($campos)
    {
        $camposValidos = [
    ...
    public function actualizarExentoAnoFisc($campos, $filtros)
    {
        $camposValidos = [
            'estatus' => [
                'tipoDato' => 'INT',
        ...
        try {
            ...
            if ($filasAfectadas >= 1) {
                ...
                error_log("[$timestamp] app/Models/Proveedores/Excepciones/ExentoAnoFisc_Mdl.php -> Se " . $estatus . " El Proveedor: " . $idProveedor . ", IdUserReg: " . ($_SESSION['EQXident'] ?? 0) . PHP_EOL, 3, $logFile);
            ...
            $timestamp = date("Y-m-d H:i:s");
            error_log("[$timestamp] app/Models/Proveedores/Excepciones/ExentoAnoFisc_Mdl.php ->Error al actualizar: " . $e->getMessage() . PHP_EOL, 3, LOG_FILE_BD);
            return ['success' => false, 'message' => 'Error: ' . $e->getMessage(), 'filasAfectadas' => 0];
        }
    }
}
