...
class Anticipos_Mdl
{
    ...
    public function verificaAnticipoDeOrdenCompra($filtros = [], INT $cantMaxRes = 0, $orden = 'DESC')
    {
        ...
        $filtrosDisponibles = [
            'folioCompra' => ['tipoDato' => 'STRING', 'sqlFiltro' => 'OC = :folioCompra']
        ];
        ...
        try {
            ...
            $sql = "SELECT * FROM vw_ext_PortalProveedores_PagosAnticipados WHERE $filtrosSQL";

            if (self::$debug) {
                $this->dbHES->imprimirConsulta($sql, $params, 'Lista Pagos Anticipados');
            }
            ...
            error_log("[$timestamp] app/Models/datosCompra/Anticipos_Mdl.php ->Error buscar Pagos Anticipados por Compra: " . $e->getMessage(), 3, LOG_FILE_BD);
            if (self::$debug) {
                echo "<br>Error al listar Pagos Anticipados: " . $e->getMessage(); // Mostrar error en modo depuración
            }
